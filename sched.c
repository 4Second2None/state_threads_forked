...
int st_iopoll(struct pollfd *pds, char * buf, int nbyte, int offset, st_utime_t timeout)
{
    struct iocb *piocb[1];
    _st_thread_t *me = _ST_CURRENT_THREAD();
    memset(&(me->riocb), 0, sizeof(me->riocb));
    me->riocb.aio_data = (void *)me;
    me->riocb.aio_lio_opcode = IOCB_CMD_PREAD;
    me->riocb.aio_fildes = pds->fd;
    me->riocb.aio_buf = (void *)buf;
    me->riocb.aio_nbytes = nbyte;
    me->riocb.aio_offset = offset;
    me->riocb.aio_flags = IOCB_FLAG_RESFD;
    me->riocb.aio_resfd = aioevfd;
    piocb[0] = &(me->riocb);
    //printf("before _st_io_submit:aio_data:%x--aio_lio_opcode:%d--aio_filedes:%d--aio_buf:%x--aio_nbytes:%d--aio_offset:%d--aio_flags:%d--aio_resfd:%d\n", me->riocb.aio_data, me->riocb.aio_lio_opcode, me->riocb.aio_fildes, me->riocb.aio_buf, me->riocb.aio_nbytes, me->riocb.aio_offset, me->riocb.aio_flags, me->riocb.aio_resfd);
    int k = _st_io_submit(aio_ctx, 1, piocb);
    if(k != 1)
    {
