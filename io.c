...
int st_iofd_close(_st_netfd_t *fd)
{
    st_netfd_free(fd);
    return close(fd->osfd);
}
...
int st_iofd_poll(_st_netfd_t *fd, int how, void* buf, size_t nbyte, int offset, st_utime_t timeout)
{
    struct pollfd pd;
    int n;
    pd.fd = fd->osfd;
    pd.events = (short)how;
    pd.revents = 0;
    if((n = st_iopoll(&pd, buf, nbyte, offset, timeout)) < 0)
    {
        return -1;
    }
    return 0;
...
static _st_netfd_t* _st_iofd_new(int osfd, int nonblock, int is_socket)
{
    _st_netfd_t *fd;
    if(_st_netfd_freelist){
        fd = _st_netfd_freelist;
...
_st_netfd_t *st_ioopen(const char * path, int flags, mode_t mode)
{
    int osfd, err;
    _st_netfd_t *newfd;
    while((osfd = open(path, flags, mode)) < 0)
    {
        if(errno != EINTR)
          return NULL;
    }

    newfd = _st_iofd_new(osfd, 0, 0);
    if(!newfd){
        err = errno;
        close(osfd);
        errno = err;
...
int st_ioread(_st_netfd_t *fd , void * buf, size_t nbyte, int offset, st_utime_t timeout)
{
     _st_thread_t *me = _ST_CURRENT_THREAD();
    int n = st_iofd_poll(fd, POLLIN, buf, nbyte, offset, timeout);
    if(n == -1)
      return -1;
    //printf("after read thread:%x---res:%d--res2:%d\n", me, me->res, me->res2);
    return me->res;
}
