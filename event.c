...
//int aioevfd = -1;
ST_HIDDEN int _st_io_setup(int maxaioev, aio_context_t * ctx)
{
//    return io_setup(maxaioev, ctx);
    return syscall(__NR_io_setup, maxaioev, ctx);
}

ST_HIDDEN int _st_io_destroy(aio_context_t ctx)
{
    //return io_destroy(ctx);
    return syscall(__NR_io_destroy, ctx);
}

ST_HIDDEN int _st_io_getevents(aio_context_t ctx_id, long min_nr, long nr, struct io_event *events, struct timespec *timeout)
{
    return syscall(__NR_io_getevents, ctx_id, min_nr, nr, events, timeout);
}

int _st_io_submit(aio_context_t a_ctx, long n, struct iocb **paiocb)
{
    return syscall(__NR_io_submit, a_ctx, n, paiocb);
}

ST_HIDDEN struct pollfd * _st_aio_init()
{
    aioevfd = eventfd(0, 0);
    if(aioevfd == -1)
      return NULL;
    int n = 1;
    if (ioctl(aioevfd, FIONBIO, &n) == -1) 
    {
        printf("aioevfd ioctl error!\n");
    }
    if(_st_io_setup(8192, &aio_ctx) == -1)
      return NULL;
    struct pollfd *evfd = (struct pollfd *)malloc(sizeof(struct pollfd));
    evfd->fd = aioevfd;
    evfd->events = POLLIN;
    evfd->revents = 0;
    return evfd;
...
ST_HIDDEN int _st_epoll_init(void)
{
    ...
    struct pollfd * evpd;
    if ((evpd = _st_aio_init()) == NULL)
      goto cleanup_epoll;
    else
    {
        _st_epoll_data->eventfd = evpd;
        if((* _st_eventsys->pollset_add)(evpd, 1) < 0)
        {
            free(evpd);
            goto cleanup_epoll;
        }
...
ST_HIDDEN void _st_eventfd_handler(int evfd)
{
    int n, events;
    uint64_t ready;
    ...
    while(ready)
    {
        events = _st_io_getevents(aio_ctx, 1, 64, event, &ts);
        if(events > 0)
        {
            ready -= events;
            int i;
            for(i = 0; i < events; i++)
            {
                _st_thread_t * thread = (_st_thread_t *)event[i].data;
                if (thread->flags & _ST_FL_ON_SLEEPQ)
                    _ST_DEL_SLEEPQ(thread);
                thread->state = _ST_ST_RUNNABLE;
                thread->res = event[i].res;
                thread->res2 = event[i].res2;
                _ST_ADD_RUNQ(thread);
                //printf("thread:%x--res:%d--res2:%d--obj:%x\n", thread, thread->res, thread->res2, event[i].obj);
            }
        }
    }
}
...
ST_HIDDEN void _st_epoll_dispatch(void)
{
    ...
    if (nfd > 0) {
        for (i = 0; i < nfd; i++) {
            ...
            if(_st_epoll_data->eventfd->fd == osfd)
              _st_eventfd_handler(osfd);
        }
